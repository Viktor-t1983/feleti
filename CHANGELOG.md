...
### Added (сессия 11, 2026-06-03 — Продукты + партии + рецепты по slug)
- **Продукты (frontend):** страница `/products` (список с фильтрацией по категориям, поиск), страница `/products/[slug]` (детали: хранение, описание, GOST)
- **Продукты (backend):** эндпоинт `GET /products/by-slug/{slug}`, добавлен поиск `?q=` в список продуктов
- **Партии (frontend):** страница `/batches` (список с фильтрами: все/активные/запланированные/завершённые, поиск, сортировка, автообновление), страница `/batches/[id]` (детали: фазы, управление start/pause/resume/cancel/complete)
- **HMI:** компоненты PhaseProgress (таймлайн фаз, обратный отсчёт), TemperatureGauge (пульсация тревоги), HmiDashboard (панель тревог, сетка 2×2/4)
- **Рецепты (frontend):** страницы рецептов переписаны на by-slug, добавлены ингредиенты, посол, таблица с дымом/щепой
- **Seed:** 65 рецептов, 58 продуктов, 58 ингредиентов (seed_recipes.py)
...
- **Созданы 9 новых SKILL.md** (в `.opencode/skills/`):
  - `db-migrations` — Alembic: генерация, применение, откат миграций
  - `frontend-init` — инициализация Next.js 14 + shadcn/ui + PWA
  - `recipe-calc` — формулы БЖУ, себестоимости, yield, потери по типу копчения
  - `pytest-testing` — структура тестов, fixtures, примеры юнит + интеграционных
  - `frontend-api-client` — axios + TanStack Query + Zustand auth + обработка ошибок
  - `program-visualizer` — визуализация программ: timeline, графики температуры, прогресс
  - `quality-control` — органолептика, лабораторные показатели, дефекты, акт КК
...
- **HMI анализ Ижицы** — глубокое исследование панели управления конкурента:
  - `docs/research/ijiza/HMI_ANALYSIS.md` — анализ HMI Ижицы-Z115 (паспорт, видео, сайт)
...
- **Новый SKILL.md:** `hmi-chamber` — спецификация HMI панели FELETI-SMOK
  ...
  - Modbus TCP карта регистров
  - Алерты и безопасность
