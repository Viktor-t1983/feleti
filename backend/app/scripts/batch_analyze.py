"""Пакетный AI-анализ всех необработанных статей.

Запуск: `docker compose exec backend python -m app.scripts.batch_analyze`
Идемпотентен — повторный запуск пропускает уже проанализированные.
"""
...
async def get_unanalyzed_ids() -> list[int]:
    async with AsyncSessionLocal() as db:
        analyzed = select(ArticleAnalysis.article_id)
        stmt = (
            select(KnowledgeArticle.id)
            .where(not_(KnowledgeArticle.id.in_(analyzed)))
...
async def main():
    start = time.time()
    logger.info("Loading unanalyzed article IDs...")

    all_ids = await get_unanalyzed_ids()
    total = len(all_ids)
    logger.info("Found %d unanalyzed articles", total)
