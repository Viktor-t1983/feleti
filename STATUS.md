...
- [x] `/matrix` — Матрица совместимости (продукт ↔ камера ↔ посол)
...
- [x] `GET /matrix/compatibility` — анализ совместимости продукта с камерами и рассолами
...
- [x] Матрица совместимости: серверная часть (GET /matrix/compatibility, service)
  - [x] Rule-based: ProductCategory ↔ ChamberType (14 категорий × 7 типов)
  - [x] Preferred brine methods по категории продукта
  - [x] Референсные рецепты для продукта (из существующих данных)
  - [x] Диапазон рабочих температур по типу камеры
- [x] Матрица совместимости: UI (/matrix)
  - [x] Селектор продукта, таблица камер (совместимые/нет с цветовой индикацией)
  - [x] Таблица рассолов (рекомендованные / неподходящие)
  - [x] Референсные рецепты (карточки с yield, фазами, длительностью)
  - [x] Ссылка в боковом меню
- [ ] Интеграция с HMI
