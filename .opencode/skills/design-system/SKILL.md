...
- `telemetry-websocket` — для live-графиков телеметрии.
- `batches-lifecycle` — для UI управления партиями.
- `knowledge-search` — для UI базы знаний.
- `add-recipe` — для UI конструктора рецептов.
